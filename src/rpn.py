...
class RPN:
    ...
    def build_model(self):
        ...
        clipped_boxes = self.clip_boxes(pred_decoded, feature_maps_shape)
        # TODO: How will this impact the loss function? Remove zero-area boxes?
    ...
    @staticmethod
    def decode_offsets(offsets, anchors, fm_shape):
        """
    ...
    def non_maximum_suppression(self, boxes, roi_scores):
        ...
        def single_image_nms(img_boxes, scores):
            selected_indices = tf.image.non_max_suppression(img_boxes, scores,
            ...
            selected_img_boxes = tf.gather(img_boxes, selected_indices)
            return selected_img_boxes

        # NMS expects the boxes to be in the format (y1, x1, y2, x2)
        reshaped_boxes = tf.stack([boxes[:, :, 1], boxes[:, :, 0], boxes[:, :, 3], boxes[:, :, 2]], axis=-1)
        reshaped_scores = tf.reshape(roi_scores,
                                     (-1, 8 * 8 * 64))  # Shape: (batch_size, 8, 8, 64) -> (batch_size, 8*8*64)
        ...
        selected_boxes = tf.stack([selected_boxes[:, :, 1], selected_boxes[:, :, 0],
                                   selected_boxes[:, :, 3], selected_boxes[:, :, 2]], axis=-1)
